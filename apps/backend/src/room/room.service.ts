...
/**
 * 사람이 읽기 쉬운 4자리 방 코드를 생성
 * 혼동되기 쉬운 문자(I, O, 1, 0 등)는 제외
 */
...
/**
 * 메모리 기반 방 상태 저장소.
 * 방 조회/생성 및 이벤트 디스패치를 통해 FSM 상태 변경을 적용
 */
export class RoomService {
  ...
  /** 소켓 ID로 해당 소켓이 속한 방의 ROOMSTATE 조회 */
  ...
  /**
   * 방 상태에 게임 이벤트를 적용
   * 성공 시 FSM 결과를 저장하고 소켓-방 인덱스를 최신 상태로 재구성
   */
  ...
  /**
   * 이벤트 적용 전/후 플레이어 소켓 목록을 비교해 소켓-방 인덱스를 갱신
   * 퇴장한 소켓 매핑은 제거하고, 남아 있는 소켓은 현재 방 코드로 재등록
   */
